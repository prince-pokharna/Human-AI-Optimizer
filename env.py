...
class StrategicRoutingEnv:
    def __init__(self, budget=50.0, max_steps=10):
        ...
        self.tasks_database = [
            ...
            {'name': 'Fraud Detection', 'base_comp': 0.9, 'base_urg': 0.8, 'cost_h': 20.0}
        ]
        self.reset()
    ...
    def step(self, action: Action):
        ...
        cost = task.get('cost_h', 10.0) if action.strategy == 'human_expert' else 1.0
        ...
        self.budget -= cost
        if not success: self.risk_level += 0.2

        self.history.append(f"{task['name']}:{action.strategy}")
        self.current_step += 1
        done = self.current_step >= self.max_steps or self.budget <= 0 or self.risk_level >= 1.0
